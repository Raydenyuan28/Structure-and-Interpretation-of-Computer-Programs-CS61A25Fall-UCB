...
class VendingMachine:
    ...
    def vend(self) -> str:
        ...
        if self.stock==0:
            ...
            k=self.balance-self.price
            self.balance=0
            self.stock-=1
            return f'Here is your {self.product} and ${str(k)} change.'
        else:
            self.balance=0
            self.stock-=1
...
def delete(t:Tree, x):
    ...
    for b in t.branches:
        delete(b,x)
        if b.label == x:
            new_branches.extend(b.branches)#append会单独加入【】，extend则会忽略【】，变成加入无
        else:
