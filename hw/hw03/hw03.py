...
def digit_distance(n):
    ...
    if n//10==0:
        return 0
    return digit_distance(n//10)+max(n%10-(n//10)%10,(n//10)%10-n%10)
...
def interleaved_sum(n, f_odd, f_even):
    ...
    def detail(i,f1,f2):
        if i-1==n:
            return 0
        return detail(i+1,f2,f1)+f1(i)
    return detail(1,f_odd,f_even)
...
def count_dollars(total):
    ...
    def detail(m,x):
        ...
        if m<0 or x==None:
            return 0
        return detail(m-x,x)+detail(m,next_smaller_dollar(x))
    if total//100>0:
        return detail(total,100)
    elif total>=50:
        return detail(total,50)
    elif total>=20:
        return detail(total,20)
    elif total>=10:
        return detail(total,10)
    elif total>=5:
        return detail(total,5)
    else:
        return detail(total,1)
